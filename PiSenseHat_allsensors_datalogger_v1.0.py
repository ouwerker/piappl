...
with open(complete_name,'a', newline='') as csvfile:
    fieldnames = ['sensorname','T-unit','T-unit','P-unit','H-unit','M-unit','M-unit','M-unit','A_UNIT','A_UNIT','A_UNIT','duration','timeunit']
    line_writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    line_writer.writeheader()
    line_writer.writerow({'sensorname':SENSORNAME,'T-unit':T_UNIT,'T-unit':T_UNIT,'P-unit':P_UNIT,'H-unit':H_UNIT,'M-unit':M_UNIT,'M-unit':M_UNIT,'M-unit':M_UNIT,'A_UNIT':A_UNIT,'A_UNIT':A_UNIT,'A_UNIT':A_UNIT,'duration':DURATION,'timeunit':TIMEUNIT})
    line_count += 1
...
with open(complete_name,'a', newline='') as csvfile:
    fieldnames = ['time','Thum','Tpre','pressure','humidity','x_magnetic_field','y_magnetic_field','z_magnetic_field','xacceleration','yacceleration','zacceleration']
    line_writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    line_writer.writeheader()
    line_count += 1 
...
#    TIMEARRAY.extend([voortgang])
    ...
    with open(complete_name,'a', newline='') as csvfile:
        fieldnames = ['time','Thum','Tpre','pressure','humidity','x_magnetic_field','y_magnetic_field','z_magnetic_field','xacceleration','yacceleration','zacceleration']
        line_writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        line_writer.writerow({'time':voortgang,'Thum': Thum,'Tpre':Tpre,'pressure':pressure,'humidity':humidity,'x_magnetic_field':xmag,'y_magnetic_field':ymag,'z_magnetic_field':zmag,'xacceleration':xacc,'yacceleration':yacc,'zacceleration':zacc})
        line_count += 1
        
    print(f'Time {voortgang:.2f} {SENSORNAME} {SENSORMODALITY} {Tpre:.2f} {T_UNIT} {Thum:.2f} {T_UNIT} {pressure:.3f} {P_UNIT} {humidity:.1f} {H_UNIT} Magnetic xyz {xmag:.1f} {ymag:.1f} {zmag:.1f} {M_UNIT} Acc xyz {xacc:.3f} {yacc:.3f} {zacc:.3f} {A_UNIT}')
